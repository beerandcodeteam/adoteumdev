        ...
        <div class="md:col-span-1">
            <div class="flex flex-col items-center justify-center space-y-2">
                @if($this->imageProfile)
                    <img class="inline-block h-24 w-24 rounded-full" src="{{ $this->imageProfile->temporaryUrl() }}" alt="" />
                @else
                    <img class="inline-block h-24 w-24 rounded-full" src="{{ $loggedUser['profile']['avatar'] }}" alt="" />
                @endif
                <div class="flex flex-row w-1/3">
                    <label
                        class="w-full flex flex-row items-center justify-center px-2 h-10
                            bg-white rounded-md shadow-md tracking-wide uppercase
                            border border-blue cursor-pointer hover:bg-primary-100
                            hover:text-white text-primary-100 ease-linear transition-all
